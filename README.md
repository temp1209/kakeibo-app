...
「起動から撮影までの動線を最速にする」を最優先に据え、記録の本線から設定操作を切り離しました。1か月の実利用フィードバックでも、**起動 → 撮影の体験は想定どおり**と評価されています。実利用で「触ったことがない」と分かった設定項目（通知の細分化・予算の集計対象・APIキー疎通ボタン）は2026-07-27のUI/UXブラッシュアップで簡略化・削除しました。
