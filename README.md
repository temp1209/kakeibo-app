...
| **手入力** | レシート以外の支出を手動追加 |
| **バックアップ** | Google Drive App folder へ JSON エクスポート/インポート（OAuth、`drive.appdata` 最小権限） |
...
| 画像 | Coil、JPEG 圧縮 |
| 認証・バックアップ | Google Sign-In、Drive REST API |
...
4. Drive バックアップを使う場合は [`docs/EXTERNAL_SETUP.md`](docs/EXTERNAL_SETUP.md) の OAuth 設定が必要
