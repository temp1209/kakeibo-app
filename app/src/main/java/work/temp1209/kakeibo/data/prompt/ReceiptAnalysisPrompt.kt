...
object ReceiptAnalysisPrompt {
    ...
    private fun extractionAndClassification(today: LocalDate): String = """
        ...
        |- **基本方針**: レシート・領収・決済画面には店舗やレジ機種によって多様な日付表記が使われる。次に挙げるのは代表例に過ぎない。**「これは会計日・取引日・購入日時を表す数字/記法だ」と判断できるものは、桁数や記法が下記の例と完全一致しなくても、文脈（"年月日" "会計日" "取引日" "ご利用日" 等のラベル、区切り記号、隣接する時刻表記、曜日表記など）から年・月・日を正しく特定し、必ず日付として扱うこと**。逆に、レシート管理番号・取引ID・電話番号・バーコード下の数字など、**日付ではない数字列を日付と誤認して転記しない**。
        |  - 4桁西暦（`YYYY/MM/DD` `YYYY-MM-DD` `YYYY年MM月DD日` `YYYY.MM.DD` など）はそのまま読む。
        ...
        |  - **8桁連続数字 `YYYYMMDD`**（例: `20260804`）も同様に西暦4桁+月2桁+日2桁として読む。
        |  - `令和6年8月4日` `R6.8.4` のように元号が明記されている場合のみ、元号→西暦変換（令和1年=2019年、令和 n 年=2018+n 年）を行う。元号の明記がない数字列を、勝手に元号年として扱わない。
        |  - `8/4` `8月4日(火)` のように**年が印字されていない月日のみの表記**は、年を捏造せず、上記の参考日付や画像内の他の手がかり（会員証・注文番号・別欄の西暦など）から矛盾のない年を推定してよい。推定した場合は confidence を下げ、warnings に `[日時]` で「年は推定」と短く書く。
