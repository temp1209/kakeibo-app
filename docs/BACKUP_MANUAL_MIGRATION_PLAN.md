...
| 未スキップ | `dismissedPromptYm` が今月と異なる（「今月は表示しない」未選択） |

今月すでにエクスポート済みなら **出さない**。
...
| 副ボタン | **あとで** → 閉じる |
| チェックボックス | **今月は表示しない** → `dismissedPromptYm` を今月に設定 |

#### 実装

| パス案 | 役割 |
|--------|------|
| `FileBackupPrefs.dismissedPromptYearMonth` | 今月スキップフラグ |
