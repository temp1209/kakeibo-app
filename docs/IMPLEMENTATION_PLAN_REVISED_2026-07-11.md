...
**Phase 7 は `feat/phase7` で完了**。`main` は保護ブランチのため **PR マージ**。

| 手順 | 状態 |
|------|------|
| `feat/phase7` push | ✅ |
| PR 作成 | ⬜ https://github.com/temp1209/kakeibo-app/pull/new/feat/phase7 |
| マージ後 | ローカル `main` を `git pull` で同期 |
...
| ブランチ / 状態 | 内容 | 次のアクション |
|-----------------|------|----------------|
| `main`（ローカル） | Phase 6 + **Phase 7 完了** | PR マージ後 `git pull` |
| `feat/phase7` | Phase 7 一式（push 済み） | PR 作成 → マージ |
| `origin/main` | Phase 6 マージ済みまで | PR マージで Phase 7 反映 |
