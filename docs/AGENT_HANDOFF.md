...
- Pixel 8aでの実機受信・インストールも確認済み。さらに、debug署名鍵をCI/ローカルで固定する修正（PR #18、下記）後、2回目の配布更新で**データが保持されること**（アンインストール→再インストールにならないこと）も実機で確認済み（2026-08-16）
- 詳細手順: `docs/EXTERNAL_SETUP.md` §5

### Firebase配布更新でデータが全消去される問題（2026-08-16・発見・修正・解決済み）

配布のたびにアプリのDB・権限・APIキー設定が全て消えることが実利用で発覚。原因はCIランナーがビルドのたびに異なるdebug署名鍵を自動生成していたため（Androidは署名者が異なるAPKへの更新を許可せず、アンインストール→再インストールを要求する）。

- このプロジェクト専用の固定debug鍵を生成し、GitHub Secrets `DEBUG_KEYSTORE_BASE64` に登録してCI実行時にのみ復元する方式で解決（PR #18マージ済み）。鍵の実体はリポジトリにコミットしていない
- 途中、開発機の複数プロジェクト共有のグローバル鍵をそのままコミットしかける失敗があったが、ユーザーの指摘で気づき撤回・是正した
- `android.yml`（test/lint検証用CI、Firebase配布とは無関係）は固定鍵ファイルが無いためAGP既定の自動生成debug署名にフォールバックするようにした（`app/build.gradle.kts`）
- 2回目の配布更新で実機データが保持されることを確認済み。以降の配布更新も同じ鍵が使われ続けるはず
