...
Phase 7 完了・M2 中核（5.2 解析状態・2.5 通知履歴）完了後、**Phase 8 品質ブラッシュアップ** に着手する計画。
...
| **2.5** | 通知履歴の永続化 | 実装済み（実機確認待ち） |

### Phase 7 — **完了**（PR #3 マージ済み）

オンボーディングの UI ブラッシュアップ・§13 改善は **Phase 8** に移管（[`plans/phase-8-polish.md`](plans/phase-8-polish.md)）。

### 次のマイルストーン — **Phase 8 品質ブラッシュアップ**

| サブ | 内容 | 規模 |
|------|------|------|
| 8.1 | Gemini JSON 閲覧改善 | S |
| 8.2 | オンボーディング UI 磨き込み | S〜M |
| 8.3 | オンボーディング §13 M1/M2 | S |
| 8.4 | §13 低優先（任意） | 任意 |
...
| 由来 | 項目 |
|------|------|
| Phase 8 | JSON 閲覧、オンボーディング UI・技術改善 |
| Phase 5.1 | プロンプト継続チューニング（**継続トラック**・Phase 8 と並行） |
| M3 | ポートフォリオ整備（任意） |
| 将来 | オフライン、検索、バックアップ暗号化等 |
...
2. **M2 中核は完了** — 解析状態可視化・通知履歴永続化。
3. **次は Phase 8** — 小さな UX/調査改善を1バンドルにまとめて仕上げる。
4. **5.1 は並行** — プロンプトは実利用ドリブンで継続。Phase 8 PR には混ぜない。
5. **push / PR** — ローカル `main` に 5.2/2.5 コミットが溜まっている場合は Phase 8 着手前に反映を検討。
...
- [ ] **Gemini JSON 閲覧改善** → **Phase 8.1**（[`plans/phase-8-polish.md`](plans/phase-8-polish.md)）
...
| 8.4 | §13 低優先（L1/L4 等・任意） | 任意 |

**完了条件**: 調査用 JSON が実用レベルで読める。Wizard の粗さと権限エッジケースが減る。

**後回しからの移管**: [`plans/onboarding.md`](plans/onboarding.md) §12–13、[`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) §4
...
| 順 | 項目 | Phase | 規模 | 状態 |
|----|------|-------|------|------|
| — | M2 中核（5.2 / 2.5） | 5.2 / 2.5 | — | ✅ 実装済み |
| 0 | 2.5 実機確認（未なら） | 2.5 | S | 任意 |
| **1** | **Gemini JSON 閲覧** | 8.1 | S | **次** |
| **2** | **オンボーディング M1/M2** | 8.3 | S | 未着手 |
| **3** | **オンボーディング UI** | 8.2 | S〜M | 未着手 |
| 4 | §13 低優先（L1 等） | 8.4 | 任意 | 未着手 |
| — | プロンプト継続チューニング | 5.1 | 継続 | 並行 |
| — | ポートフォリオ整備 | M3 | 任意 | 未着手 |
...
**出口**: 調査・初回体験の「粗さ」が解消。コアフローの回帰なし。

詳細: [`plans/phase-8-polish.md`](plans/phase-8-polish.md)
...
- [`plans/phase-8-polish.md`](plans/phase-8-polish.md) — **Phase 8 品質ブラッシュアップ（次）**
