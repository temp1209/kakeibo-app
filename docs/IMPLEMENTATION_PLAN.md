...
### 次のマイルストーン — **Phase 10〜11**（2026-07-15）

| Phase | 内容 | 状態 |
|-------|------|------|
| **9.5** | ブレスト + 要件定義 | ✅ ドキュメント（PR #6） |
| **10** | 複数 AI / API・フェイルオーバー | ✅ PR #7 マージ済み |
| **11** | 予算・通知・分析 UI・失敗 UX | ✅ 完了（`main` マージ） |
...
| 判断 | 理由 |
|------|------|
| **送信前プレビューの回転/トリミングは実装しない** | コアの3操作（起動 → シャッター → 送信確定）に確認・修正ステップを挟むと、手入力家計簿から逃げてきた「毎回の手間」を再導入してしまう。また、傾き・しわのあるレシートへの対応はGemini解析側の頑健性で担保する設計方針（§「プロンプトエンジニアリングによる構造化OCR」）であり、撮影後の画像前処理で解決すべき問題ではない。1か月以上の実利用でも読み取り精度の実害は発生していない |
| **バックアップ（JSON）の暗号化は実装しない** | 写真を含まない文字データのみで、ローカル保管に限定して運用しているため強い機密性は不要と判断。加えて、将来的に他アプリやAIにバックアップJSONを読ませる用途も想定しており、暗号化は復号の手間を増やすだけでその想定用途と相反する |
| **オフライン対応は「大きな新機能」ではなく既存実装の確認＋2点の穴埋め** | 調査の結果、撮影・キュー投入（`ReceiptRepository.savePendingReceipt`/`enqueueAnalysis`）はネットワーク状態を一切見ておらず常に成功し、`scheduleAnalysisWork()` が `Constraints(NetworkType.CONNECTED)` 付きで `WorkManager` に `enqueueUniqueWork(..., ExistingWorkPolicy.KEEP, ...)` している時点で「オフラインでキュー保持→通信復帰後に自動解析」は既に成立していた（WorkManager 側で端末再起動を跨いで永続化される）。実装済みだったのは、①一覧/詳細の状態ラベルが `PENDING`＝「解析待ち」/`RUNNING`＝「解析中」で既に区別されていたこと（Phase 5.2）。未対応だったのは②画像40日retentionのクリーンアップ（`cleanupExpiredImages`）が `analysisStatus` を見ておらず、長期オフラインで解析未完了のまま40日を過ぎると元画像が消え、通信復帰後の解析が「画像がありません」で失敗し得た点のみ。②を `isImageCleanupEligible()` で修正し、`PENDING`/`RUNNING` のレシートをクリーンアップ対象から除外した |
| **未処理キューの「無期限のオフライン許容」はしない。7日で異常（バグ）とみなして解析失敗に確定** | 実利用では通信が長期間繋がらない状況はまず発生しないため、7日以上未処理のキューはオフラインより「バグでスタックしている」可能性の方が高いと判断（フィードバックにより方針転換）。②の穴埋めだけでは `PENDING`/`RUNNING` が無期限に画像・キューへ残り続けるゴミになりかねないため、`failStaleQueueEntries()` を追加し、投入から7日超のQUEUED/RUNNINGは解析失敗として確定し既存の失敗一覧・通知フローに乗せる。判定ロジックは `isQueueEntryStale()` として純粋関数に切り出しユニットテスト済み |

---

## 全体方針（2026-07-15）

1. **コア体験は完成** — 撮影 → 解析 → 一覧/分析 + 手動バックアップ + 初回セットアップ + 必須度ポリシー。
2. **Phase 10** — 複数 Gemini キーのフェイルオーバー（PR #7 マージ済み）。
3. **Phase 11** — 実装・自動検証完了。実機確認後に PR。並行で 5.1 プロンプト。
4. **スコープ拡大は控えめ** — 8.4 残・第 2 プロバイダ種は必要時のみ。
...
| **1** | **複数 AI / API** | 10 | ✅ PR #7 マージ済み |
| **2** | **予算・通知・分析 UI** | 11 | ✅ 完了（`main` マージ） |
| — | プロンプト継続チューニング | 5.1 | 実利用並行 |
| — | UI/UX ブラッシュアップ（設定整理など） | — | 後続 |
...
- [x] Phase 10 複数 AI / API・フェイルオーバー（実装・実機確認）
- [x] Phase 11 予算・通知・分析 UI（完了・`main` マージ）

詳細: [`plans/phase-9.5-brainstorm.md`](plans/phase-9.5-brainstorm.md) / [`plans/phase-10-multi-ai-provider.md`](plans/phase-10-multi-ai-provider.md) / [`plans/phase-11-budget-notifications.md`](plans/phase-11-budget-notifications.md)
...
- [x] ~~オフライン対応（撮影 → キュー保持 → 後解析）~~ — 2026-07-27 完了（下記「主な判断」参照）
- [ ] ~~送信前プレビューの回転/トリミング~~ — 2026-07-27 不要と判断（下記「主な判断」参照）
- [x] ~~検索（店名/商品名）~~ — 2026-07-27 完了。一覧タブ右上の検索アイコンをタップで検索欄を開閉するトグル式（常時表示ではない）。全期間対象で`ReceiptDao.searchReceiptRows`により部分一致検索。検索欄の表示状態・検索語は`MainActivity`側にホイストし、レシート詳細画面への遷移・復帰をまたいで保持
- [ ] ~~バックアップ暗号化（パスフレーズ）~~ — 2026-07-27 不要と判断（下記「主な判断」参照）
