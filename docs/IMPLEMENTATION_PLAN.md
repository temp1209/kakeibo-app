...
### 次のマイルストーン — **Phase 9 必須度ポリシー** ← **現在地**

| サブ | 内容 | 状態 |
|------|------|------|
| 9.1〜9.3 | 保存・コンパイル・解析注入 | ⬜ |
| 9.4 | 訂正例 UI・未反映バナー | ⬜ |
| 9.5 | 当月再スコア Worker | ⬜ |
| 9.6 | バックアップ 1.2 | ⬜ |

詳細: [`plans/phase-9-necessity-policy.md`](plans/phase-9-necessity-policy.md) / 要件: `REQUIREMENTS.md` §16

### 並行トラック

| トラック | 内容 |
|----------|------|
| **5.1** | プロンプト・回帰素材（実利用ドリブン・Phase 9 PR と分離） |
| **8.4 残** | L4/L6 等（任意・必要時のみ） |
| **M3** | ポートフォリオ整備（任意） |
...
**完了条件**: ユーザーが方針を設定し、解析・当月再スコア・バックアップが一貫して動作する。
...
### M3 — Phase 9 必須度ポリシー ← **現在地**

- [ ] 方針コンパイル（ハイブリッド）+ 設定 UI
- [ ] 解析・当月再スコアへの反映
- [ ] バックアップにユーザー設定を含める
...
- [`plans/phase-9-necessity-policy.md`](plans/phase-9-necessity-policy.md) — **Phase 9 必須度ポリシー（次）**
