...
### 次のマイルストーン — **Phase 9 必須度ポリシー**（実装完了・実機・PR 待ち）

| サブ | 内容 | 状態 |
|------|------|------|
| 9.1〜9.6 | 保存・コンパイル・解析注入・訂正例・再スコア・バックアップ | ✅ |
| 9.7 | 実機スモーク・回帰 | ⬜ |
...
**完了条件**: ユーザーが方針を設定し、解析・当月再スコア・バックアップが一貫して動作する（実機確認残）。
...
- [`plans/phase-9-necessity-policy.md`](plans/phase-9-necessity-policy.md) — **Phase 9 必須度ポリシー（実装済み・実機待ち）**
