...
- [ ] categoryMajor と categoryMinor の対応関係バリデーション（Phase3で強化）
...
### 3.1 一覧/詳細UIを要件へ寄せる
- [ ] 一覧: 日時/店名/合計/必須vs裁量バッジ
- [ ] 詳細: 明細行表示（元順序、調整行はUIに出さない）
- [ ] needsReview時のみ状態表示、欠落や低信頼の強調

### 3.2 必須vs裁量（集計ロジック）
- [ ] necessityScoreの金額加重平均（調整行除外）
- [ ] 分析タブ: 必須vs裁量比率、無駄遣い候補TOP、月切替

### 3.3 低信頼（要確認）修正フロー
- [ ] 修正画面（必須項目 + 支払手段 + 必須度スコア）
- [ ] 必須欠落の定義固定（要件準拠）
- [ ] 修正完了で needsReview解除 → 集計対象へ
